...
async function editComment(commentId) {
    try {
        const comment = await commentsAPI.getByPost(commentId.split('_')[0]);
        const currentComment = comment.find(c => (c._id || c.id) === commentId);
        if (!currentComment) return;
        
        const newContent = prompt('Edit comment:', currentComment.content);
        if (newContent && newContent.trim()) {
            await commentsAPI.update(commentId, newContent.trim());
            await showPostDetails(currentComment.postId);
        }
    } catch (error) {
