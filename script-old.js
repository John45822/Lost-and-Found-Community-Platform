...
signupFormElement.addEventListener('submit', (e) => {
    ...
    if (users.some(u => u.username === username)) {
        statusMessage.textContent = 'Username already exists. Please choose a different username.';
        statusMessage.style.color = '#dc3545';
        return;
    }
    ...
    convertImageToBase64(idPictureFile, (base64Image) => {
        ...
        // Automatically notify admin about the activation request
        const admin = users.find(u => u.role === 'admin');
        if (admin) {
            createNotification(admin.id, `New account activation request from ${newUser.fullName} (${newUser.username})`, 'account_request');
        }
        
        statusMessage.textContent = 'Registration successful! Your account activation request has been sent to the admin. Please wait for approval.';
...
// Request Activation Handler - Removed (now combined with registration)
